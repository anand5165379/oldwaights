...
class Carrot_Segmentor(object):
    def __init__(self, hsv_h_low, hsv_h_high):
        self.hsv_h_low = hsv_h_low
        self.hsv_h_high = hsv_h_high
    ...
    def segment(self, np_image):
        ...
        h, _, _ = cv2.split(hsv)
        ret, thresh = cv2.threshold(h, self.hsv_h_low, self.hsv_h_high, cv2.THRESH_BINARY_INV)
    ...
    #     print(length[0][0], width[0][0], length[0][0] * width[0][0])
        ...
        if max(length[0][0], width[0][0]) > 0 and length[0][0] * width[0][0] < 500000:
