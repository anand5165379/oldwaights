...
def segment(np_image):
	...
	# cv2.imwrite("gourd/h.jpg",s)
	ret, thresh = cv2.threshold(s, 30, 255, cv2.THRESH_BINARY_INV)
	# ret, thresh = cv2.threshold(v, 27, 255, cv2.THRESH_BINARY_INV)
...
def segment_image(img):
	...
	for conti in contours:
		...
		if cv2.contourArea(conti) > 20000:
